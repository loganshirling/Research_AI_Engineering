# AI Engineering Topics

This folder holds the topic layer for the AI Engineering course.

## Where this fits

- Course hub: [[AI Engineering - Course Home]]
- Map: [[Map - AI Engineering]]
- Practice hub: [[AI Engineering Practice]]

## How to use this folder

Use topic notes when you need a practical working model for one major subdomain such as retrieval, serving, evaluation, agents, or multimodal systems.

A good rhythm is:
1. read the relevant section in [[AI Engineering - Roadmap]]
2. read one topic note here
3. review linked concepts or techniques
4. do a related practice drill
5. return to [[Map - AI Engineering]]

## Main topic clusters

### Foundations and serving
- [[Topic - LLM Inference Path, KV Cache, and Batching]]
- [[Topic - Quantization Tradeoffs for LLM Inference]]
- [[Topic - Latency Engineering for LLM Systems]]
- [[Topic - Production Bottlenecks and Observability for LLM Systems]]
- [[Topic - vLLM, PagedAttention, Prefix Caching, and Continuous Batching]]

### Retrieval and RAG
- [[Topic - Retrieval System Design for RAG]]
- [[Topic - Hybrid Search and Reranking]]
- [[Topic - Common RAG Failure Modes]]
- [[Topic - Vector Databases, Embeddings, and Retrieval Tradeoffs]]

### Prompting, tools, and workflow control
- [[Topic - Tool Calling, Structured Outputs, and Agent Loops]]
- [[Topic - Context Engineering, Prompt Caching, and Long-Context Tradeoffs]]
- [[Topic - Agent Orchestration, Handoffs, and Deterministic Workflow Design]]
- [[Topic - MCP, Tool Design, and Secure Agent Execution]]

### Evaluation, safety, and operations
- [[Topic - Eval Design for Serving Changes]]
- [[Topic - Eval Flywheels, Human Review, and Regression Gates]]
- [[Topic - Model Selection, Benchmarking, and Tradeoff Triage]]
- [[Topic - Guardrails, Prompt Injection, and Output Validation]]
- [[Topic - LLM Observability and Tracing]]

### Advanced systems and capstone patterns
- [[Topic - Advanced Systems, Agents, Multimodal, and Voice]]
- [[Topic - AI Agents: Design, Boundaries, and Operations]]
- [[Topic - Multimodal Document and Image Pipelines]]
- [[Topic - Realtime Voice Agents and Speech-to-Speech Systems]]
- [[Topic - End-to-End AI System Case Studies and Build Patterns]]
- [[Topic - OpenClaw and Self-Hosted Personal Agents]]

### Customization
- [[Topic - When to Fine-Tune vs When Not To]]
- [[Topic - Adapters, LoRA, and PEFT Tradeoffs]]

## Related folders

- Concepts: `Vault/03_Concepts/AI Engineering/`
- Techniques: `Vault/04_Techniques/AI Engineering/`
- Sources: `Vault/06_Sources/AI Engineering/`
- Practice: `Vault/08_Practice/AI Engineering/`
- Reviews: `Vault/09_Reviews/AI Engineering/`
